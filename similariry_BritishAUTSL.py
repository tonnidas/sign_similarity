...
def get_words_csv(inputDF, videoName):
  return inputDF[inputDF['Name'] == videoName]['EN'].tolist()
...
inputDF = pd.read_csv('datasets/AUTSL/train_labels_merged.csv')
outFiles = os.listdir('datasets/AUTSL/processed_AUTSL/vtt') # vtt files

for f in outFiles:
  n = n + 1

  videoName = f.removesuffix('_color_result.vtt')
  outFile = f'datasets/AUTSL/processed_AUTSL/vtt/{f}'

  inWords = get_words_csv(inputDF, videoName)
  ...
  if c > 0:
    matched += 1
    print(f'Line: {videoName}, words: {w}, matched: {c}, accuracy: {round(c*100/w, 2)}')
...
categories.to_csv('datasets/AUTSL/processed_AUTSL/output_AUTSL/category.csv', sep='\t')
