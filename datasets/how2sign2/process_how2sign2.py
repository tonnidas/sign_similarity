...
df = pd.read_csv('raw_how2sign2/srt/how2sign_realigned_test.csv', sep='\t')
df['length'] = df['END_REALIGNED'] - df['START_REALIGNED']

cnt = 0

for index, row in df.iterrows():
    if row['length'] > 1 and row['length'] < 3:
        cnt += 1

        # split srt
        time = f"{row['START_REALIGNED']} --> {row['END_REALIGNED']}"
        sentence = time + '\n' + processSentence(row['SENTENCE'])
        outfileName = f"processed_how2sign2/srt/{row['SENTENCE_NAME']}.srt"
        outfile = open(outfileName, 'w')
        outfile.write(sentence)
        outfile.close()

        # split video
        clip = VideoFileClip(f"raw_how2sign2/videos/{row['VIDEO_NAME']}.mp4")
        clip = clip.subclip(math.floor(
            row['START_REALIGNED']), math.ceil(row['END_REALIGNED']))
        clip.write_videofile(
            f"processed_how2sign2/videos/{row['SENTENCE_NAME']}.mp4", fps=25)
